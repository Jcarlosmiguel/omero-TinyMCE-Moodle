...
class qtype_omerohotspot_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        if ($clickx !== '' && $clicky !== '') {
            $result .= html_writer::div(
                get_string('currentanswer', 'qtype_omerohotspot', (object) ['x' => round((float) $clickx), 'y' => round((float) $clicky)]),
    ...
    public function correct_response(question_attempt $qa) {
        // Deliberately blank - see qtype_omerohotspot_question::get_correct_response()'s
        // own docblock for why the region stays undescribed even here.
        return '';
    }
}
